...
macro_rules! set_a1 {
    ($name:ident, $entries:ident, $alloc:ident, $val:expr) => {
        pub fn $name(vm: &mut VirtualMachine, $entries: ObjectHandle) -> RuntimeResult<ObjectHandle> {
            let $entries = vm.obj_heap.expect_set($entries)?;
...
impl ObjectSet {
    ...
    pub fn add(vm: &mut VirtualMachine, receiver: ObjectHandle, item: ObjectHandle) -> RuntimeResult<ObjectHandle> {
        ...
        let mut bucket = {
            let entries = vm.obj_heap.expect_set(receiver)?;
    ...
    pub fn __str__(vm: &mut VirtualMachine, receiver: ObjectHandle) -> RuntimeResult<ObjectHandle> {
        let all_items: Vec<ObjectHandle> = vm.obj_heap.expect_set(receiver)?.values().flat_map(|b| b.iter().copied()).collect();
    ...
    set_a1!(__bool__, entries, alloc_bool_instance, entries.values().any(|b| !b.is_empty()));

    set_a1!(__not__, entries, alloc_bool_instance, entries.values().all(|b| b.is_empty()));

    set_a1!(__len__, entries, alloc_integer_instance, entries.values().map(|b| b.len()).sum::<usize>() as i64);

    // ---- iteration protocol ----

    pub fn __iter__(vm: &mut VirtualMachine, receiver: ObjectHandle) -> RuntimeResult<ObjectHandle> {
        let items: Vec<ObjectHandle> = vm.obj_heap.expect_set(receiver)?.values().flat_map(|b| b.iter().copied()).collect();
