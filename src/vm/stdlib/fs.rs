...
// =============================================================================
//  FileData — stored in Instance via ObjectInstanceData::Native
// =============================================================================
...
impl VirtualMachine {
    ...
    fn fs_file_init(&mut self, args: &[ObjectHandle]) -> ExecuteResult<ObjectHandle> {
        ...
        let path = self.get_string_instance(path_handle)?.as_str().to_string();
        let mode = match mode_handle {
            Some(h) => self.get_string_instance(h)?.as_str().to_string(),
            None => "r".to_string(),
        };
    ...
    fn fs_file_readline(&mut self, receiver: ObjectHandle) -> ExecuteResult<ObjectHandle> {
        ...
        Ok(self.obj_heap.alloc_string_instance(ShrString::new_string(&line)))
    }
    ...
    fn fs_exists(&mut self, path: ObjectHandle) -> ExecuteResult<ObjectHandle> {
        let s = self.get_string_instance(path)?;
        let ok = std::path::Path::new(s.as_str()).exists();
        Ok(self.obj_heap.alloc_bool_instance(ok))
    }

    fn fs_is_file(&mut self, path: ObjectHandle) -> ExecuteResult<ObjectHandle> {
        let s = self.get_string_instance(path)?;
        Ok(self.obj_heap.alloc_bool_instance(std::path::Path::new(s.as_str()).is_file()))
    }

    fn fs_is_dir(&mut self, path: ObjectHandle) -> ExecuteResult<ObjectHandle> {
        let s = self.get_string_instance(path)?;
        Ok(self.obj_heap.alloc_bool_instance(std::path::Path::new(s.as_str()).is_dir()))
    }

    fn fs_remove(&mut self, path: ObjectHandle) -> ExecuteResult<ObjectHandle> {
        let s = self.get_string_instance(path)?;
    ...
    fn fs_rename(&mut self, from: ObjectHandle, to: ObjectHandle) -> ExecuteResult<ObjectHandle> {
    ...
    fn fs_write(&mut self, path: ObjectHandle, text: ObjectHandle) -> ExecuteResult<ObjectHandle> {
    ...
    fn fs_list_dir(&mut self, path: ObjectHandle) -> ExecuteResult<ObjectHandle> {
        ...
        Ok(self.obj_heap.alloc_list_instance(entries))
    }
